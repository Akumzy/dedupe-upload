...
export default class Client {
  ...
  constructor(private url: string) {
    ...
    this.#worker = URL.createObjectURL(
  ...
  sendBinary = async (
    data: { event: string; data: any; blob: ArrayBuffer },
    ack: Function,
  ) => {
    const io = await this.#io();
    let id = `${AckKey}${performance.now()}`;
  ...
  upload = async (
    file: File,
    options: { folder_id?: string; folder_path?: string; id: string } = {
      id: "upload",
    },
  ) => {
        ...
        try {
          if (p.exists) {
            ...
            for (const hash of p.blocks) {
              ...
              const blob = await ths.#readBlock({ file, start, end });
...
function appendBuffer(buf1: ArrayBuffer, buf2: ArrayBuffer) {
