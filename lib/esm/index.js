...
var _server_listener, _listener, _connection_wait, _uploads, _worker, _errors, _connect, _reconnect, _handlers, _io, _readBlock, _dedupe;
...
export default class Client {
    constructor(url) {
        ...
        this.sendBinary = async (data, ack) => {
            const io = await __classPrivateFieldGet(this, _io).call(this);
            let id = `${AckKey}${performance.now()}`;
            __classPrivateFieldGet(this, _server_listener).set(id, ack);
            // Encode meta data to Uint8Array
            const meta = new TextEncoder().encode(JSON.stringify({
                event: data.event,
                data: data.data,
                ack: id,
            }));
            // Allocate 16bits  to hold the offset of the meta data length
            const offset = new Uint16Array(1);
            offset[0] = meta.byteLength;
            let buf = appendBuffer(offset.buffer, meta);
            buf = appendBuffer(buf, data.blob);
            io.send(buf);
        };
        ...
        this.upload = async (file, options = {
            id: "upload",
        }) => {
                ...
                this.send(EVENT_CHECK_RECORD, payload, async function (p) {
                    ...
                    try {
                        if (p.exists) {
                            ...
                            __classPrivateFieldSet(ths, _errors, p);
                            ...
                            for (const hash of p.blocks) {
                                ...
                                const blob = await __classPrivateFieldGet(ths, _readBlock).call(ths, { file, start, end });
                                let res = await new Promise((resolve) => {
                                    ths.sendBinary({
                                        event: EVENT_UPLOAD,
                                        data: { upload_id: p.upload_id, hash, file_hash },
                                        blob,
                                    }, (h) => {
                                        resolve(h === hash);
                                    });
    ...
    getError() {
        const errors = __classPrivateFieldGet(this, _errors);
        __classPrivateFieldSet(this, _errors, null);
        return errors;
    }
}
_server_listener = new WeakMap(), _listener = new WeakMap(), _connection_wait = new WeakMap(), _uploads = new WeakMap(), _worker = new WeakMap(), _errors = new WeakMap(), _connect = new WeakMap(), _reconnect = new WeakMap(), _handlers = new WeakMap(), _io = new WeakMap(), _readBlock = new WeakMap(), _dedupe = new WeakMap();
